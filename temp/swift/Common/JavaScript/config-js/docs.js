...
*/
			...
			var task2 = new Ext.util.DelayedTask(function(){
				//console.log("enter delay22222222"+(Ext.get('api-tree')==null));
			   // Check for success
				//console.log('ln:'+nodes==null);
			   ...
			   if (success.two && success.one) {
				   //console.log("success true");
			      // Callback
			      //doCallback();
			   } else {
				   //alert('yyyy');
				   //console.log("delayingjhkjhkjhkjhkjhkhjjk222222222222");
			      task2.delay(50);
	...
	myTreeLoader.on("loadException", function(treeLoader, node,response) {
		//alert(node);
        //this.baseParams.category = node.attributes.category;
    }, this);
...
Ext.extend(ApiPanel, Ext.tree.TreePanel, {
    selectClass : function(cls,tp,main){
		//alert("this is the cls::"+cls);
		
		if(cls){
			var tab = main.getComponent('docs-'+cls);
			//air.trace("component id::"+tab);
			if(tab){
				if (cls.indexOf('Panel') > -1) {
					//myMask.show();				
					//Ext.Msg.progress("Wait","Laoding form");
					
					/*
					thePanel = loadClass(id);
					thePanel.id=id;
					thePanel.cclass=cls;
	            ...
	            */
				var detail = cls.substr(0,cls.indexOf('Panel'));
				...
				//air.trace("calling zone");
				//air.trace("panel id::::::::::"+cls);
				//alert('dddd----'+tp);
	        	alert('tab exists');
	        	thePanel = loadClassManually(cls,null,null);
				var oldId = thePanel.id;
				//alert('replace id::'+oldId);
				//thePanel.id='docs-'+cls;
				//air.trace("aaelect class panel id::::::::::"+thePanel.id);
				//p.add(thePanel);
				//myMask.show();
				//Ext.remove(thePanel.id);
				main.setActiveTab(thePanel);
				//Ext.Msg.updateProgress(1,'Loading','done');
				//myMask.hide();
            ...
            for(var i = 0; i < last; i++){ // things get nasty - static classes can have .
				...
				//air.trace("staticCls::"+staticCls);
                if(p == 'Ext' || !staticCls){
                    parts[i] = 'pkg-'+p;
					//air.trace("parts[i]::"+parts[i]);
                }else if(staticCls){
                    --last;
                    parts.splice(i, 1);
                }
            ...
            parts[last] = cls;
			//air.trace(parts[last]);
			//air.trace("'/root/apidocs/'+parts.join('/')::"+'/root/apidocs/'+parts.join('/'));
...
DocPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent : function(){
    	var tclass = '';
    	if(this.cclass!='undefined')
    	{
    		tclass = this.cclass;
    	}
    	if(tclass!=null && tclass!='' && tclass!='undefined' && tclass.indexOf('.'>-1))
    	{
    	//alert('tclass::'+tclass)
        var ps = tclass.split('.');
...
var main=Ext.extend(MainPanel, Ext.TabPanel, {
    ...
    loadClass : function(href, cls, member){
		...
		//air.trace("href::"+href+" cls::"+cls+" member::"+member)
    	//alert('dddd'+member);
        var id = 'docs-' + cls;
		//air.trace("id:::::::::::::::::::::::::::::::::::::::::::::"+id);
